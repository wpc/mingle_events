...
The event fetcher will manage it's own state, so fetch_latest can be called repeatedly to fetch only new events.
...
A call to reset_to_now will tell the fetcher only to pull new stuff, starting now.
